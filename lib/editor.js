...
EditorWithTags.prototype = {
  ...
  insertTag: function(data) {
    ...
    return $(node).after(this.makeTag(data));
  },
  makeTag: function(data) {
    var close;

    close = "<span class='close'>x</span>";
    return $("<span key='" + data.key + "' class='tag'>" + data.value + close + "</span>");
  },
  test: function() {
    this.elem.find(".input").append(this.makeTag({
      key: "@",
      value: "value"
    }));
    return this.elem.find(".input").append(this.makeTag({
      key: "@",
      value: "value 2"
    }));
  }
