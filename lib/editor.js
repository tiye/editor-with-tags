...
EditorWithTags = (function() {
  function EditorWithTags(options) {
    ...
    this.takeValue = __bind(this.takeValue, this);
    this.text = __bind(this.text, this);    this.options = options;
    this.elem = $("<div>").addClass("editor-with-tags");
    this.elem.append($("<div>").addClass("placeholder"));
    this.elem.find(".placeholder").append($("<span>"), $("<div>").addClass("icon"));
    this.elem.find(".icon").append($("<img>"));
  ...
  EditorWithTags.prototype.text = function(text) {
    if (text != null) {
      this.elem.find(".input").text(text);
      return this;
    } else {
      return this.elem.find(".input").text();
    }
  };

  EditorWithTags.prototype.takeValue = function() {
    var elem, index, key, ret, tags, text, value, _i, _ref;

    tags = this.elem.find("a");
    tags.remove();
    text = this.elem.find(".input").text().replace(/\s+/g, " ");
    ret = {
      text: text,
      tags: {}
    };
    for (index = _i = 0, _ref = tags.length; 0 <= _ref ? _i < _ref : _i > _ref; index = 0 <= _ref ? ++_i : --_i) {
      elem = $(tags[index]);
      key = elem.attr("key");
      value = elem.text().trim();
      ret.tags[key] = value;
    }
    this.elem.find(".input").html("");
    return ret;
  };
  ...
  EditorWithTags.prototype.onkeydown = function(event) {
    ...
    console.log(this.hasMenu, "keydown event", event.keyCode);
    if (this.hasMenu) {
  ...
  EditorWithTags.prototype.insertTag = function(data) {
    ...
    node.data = before.slice(0, -(this.piece.length + 1));
  ...
  EditorWithTags.prototype.dropMenu = function() {
    if (!this.hasMenu) {
      console.log("drop");
      this.hasMenu = true;
      return this.elem.find(".menu").show();
    }
  };
  ...
  EditorWithTags.prototype.foldMenu = function() {
    if (this.hasMenu) {
      this.hasMenu = false;
      return this.elem.find(".menu").hide();
    }
  };
  ...
  EditorWithTags.prototype.placeholder = function(text) {
    this.placeholderText = text;
    return this.maintainPlaceholder();
  };

  EditorWithTags.prototype.maintainPlaceholder = function() {
    console.log("maintain", this.elem.find(".input").text().length);
    if (this.elem.find(".input").text().length > 0) {
      return this.elem.find(".placeholder span").text("");
    } else {
      return this.elem.find(".placeholder span").text(this.placeholderText);
    }
  };

  EditorWithTags.prototype.setIcon = function(url) {
    return this.elem.find(".icon img").attr("src", url);
  };
